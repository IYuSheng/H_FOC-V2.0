...
inline void svpwm_calc_times(AlphaBetaTypeDef *alpha_beta, SVPWM_t *svpwm, float32_t vdc)
{
    ...
    switch (svpwm->sector)
    {
        case 1:  // 扇区1：V4(100) + V6(110) → T1=-Z, T2=X 3
            svpwm->T1 = -Z;
            svpwm->T2 = X;
            break;
        case 2:  // 扇区2：V6(110) + V2(010) → T1=Z, T2=Y 1
            svpwm->T1 = Z;
            svpwm->T2 = Y;
            break;
        case 3:  // 扇区3：V2(010) + V3(011) → T1=X, T2=-Y 5
            svpwm->T1 = X;
            svpwm->T2 = -Y;
            break;
        case 4:  // 扇区4：V3(011) + V1(001) → T1=-X, T2=Z 4
            svpwm->T1 = -X;
            svpwm->T2 = Z;
            break;
        case 5:  // 扇区5：V1(001) + V5(101) → T1=-Y, T2=-Z 6
            svpwm->T1 = -Y;
            svpwm->T2 = -Z;
            break;
        case 6:  // 扇区6：V5(101) + V4(100) → T1=Y, T2=-X 2
            svpwm->T1 = Y;
            svpwm->T2 = -X;
            break;
        default:
...
inline void clark_transform(void *abc_i, AlphaBetaTypeDef *alpha_beta)
{
    ...
    const float32_t ONE_SQRT3 = _1_SQRT3;
...
/**
 * @brief 速度环PID计算
 * @param target_speed 目标速度(°/s)
 * @param actual_speed 实际速度(°/s)
 * @return 输出值(Q轴电流)
 */
