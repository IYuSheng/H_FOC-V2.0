...
#define ADC1_REG_CHANNELS      3
#define ADC1_DMA_BUFFER_SIZE   (ADC1_REG_CHANNELS * 2)
#define ADC1_DMA_CHANNEL       LL_DMA_CHANNEL_2

static uint16_t adc1_dma_buffer[ADC1_DMA_BUFFER_SIZE] = {0};
static uint16_t adc1_raw_data[ADC1_REG_CHANNELS] = {0};
static uint16_t adc1_offset[ADC1_REG_CHANNELS] = {2048, 2048, 2048};
...
void MX_ADC1_Init(void)
{
  ...
  */
  ...
  // ---------- Injected group (calibration use) ----------
  ...
  DMA_InitStruct.NbData = ADC1_DMA_BUFFER_SIZE;
  DMA_InitStruct.PeriphOrM2MSrcAddress =
  LL_ADC_DMA_GetRegAddr(ADC1, LL_ADC_DMA_REG_REGULAR_DATA);
  DMA_InitStruct.MemoryOrM2MDstAddress = (uint32_t)adc1_dma_buffer;
  LL_DMA_Init(DMA1, ADC1_DMA_CHANNEL, &DMA_InitStruct);
  ...
  // NVIC_EnableIRQ(DMA1_Channel2_IRQn);  // 待FOC初始化完成后再执行
...
/**
 * @brief ADC函数（处理电流数据并迅速进行FOC计算控制）
 */
static inline void adc1_process_samples(const uint16_t *raw)
{
    foc_raw_data.ia = raw[0] - (q15_t)(adc1_offset[0]);
    foc_raw_data.ib = raw[1] - (q15_t)(adc1_offset[1]);
    foc_raw_data.ic = raw[2] - (q15_t)(adc1_offset[2]);
...
void DMA1_Channel2_IRQHandler(void)
{
    ...
    {
        for (uint8_t i = 0; i < ADC1_REG_CHANNELS; i++) {
            adc1_raw_data[i] = adc1_dma_buffer[ADC1_REG_CHANNELS + i];
        }
        LL_DMA_ClearFlag_HT2(DMA1);
        adc1_process_samples(adc1_raw_data);
    }

    if (LL_DMA_IsActiveFlag_TC2(DMA1))
    {
        for (uint8_t i = 0; i < ADC1_REG_CHANNELS; i++) {
            adc1_raw_data[i] = adc1_dma_buffer[i];
        }
...
void adc1_current_offset_calibrate(void)
{
    ...
    // 暂停DMA和Regular触发
    LL_DMA_DisableChannel(DMA1, ADC1_DMA_CHANNEL);
    ...
    for (uint32_t i = 0; i < samples; i++) {
        ...
        sum0 += LL_ADC_INJ_ReadConversionData12(ADC1, LL_ADC_INJ_RANK_1);
        sum1 += LL_ADC_INJ_ReadConversionData12(ADC1, LL_ADC_INJ_RANK_2);
        sum2 += LL_ADC_INJ_ReadConversionData12(ADC1, LL_ADC_INJ_RANK_3);
    }

    adc1_offset[0] = (uint16_t)(sum0 / samples);
    adc1_offset[1] = (uint16_t)(sum1 / samples);
    adc1_offset[2] = (uint16_t)(sum2 / samples);

    debug_log("ADC1 Current Offset Calibrated: IA=%d, IB=%d, IC=%d", adc1_offset[0], adc1_offset[1], adc1_offset[2]);
...
{
  static float vbus_filt = 0.0f;
  static uint8_t inited = 0;

  float vbus_now = adc2_raw_data[3] * voltage_conv_factor;
  ...
  if (!inited) {
      ...
      vbus_filt += alpha * (vbus_now - vbus_filt);
  }

  foc_voltage_data.vbus = vbus_filt;
